...
#define BLM_FORCE_RADIANS //use radians for angle
#include <glm.hpp>
...
private_fun:
    ...
    {
        VkCommandBufferAllocateInfo allocInfo = {}; //use command buffer to copy memory from stagingBuffer to vertexBuffer
        ...
        vkBeginCommandBuffer(commandBuffer, &beginInfo); //begin to record command buffer
        ...
        vkEndCommandBuffer(commandBuffer); //end record command buffer
        ...
        vkQueueSubmit(graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE); //submit command buffer and transmit process start
        vkQueueWaitIdle(graphicsQueue); //wait transmit finished
