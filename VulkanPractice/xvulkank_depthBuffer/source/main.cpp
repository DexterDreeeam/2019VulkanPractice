...
#define GLM_FORCE_RADIANS //use radians for angle
#define GLM_FORCE_DEPTH_ZERO_TO_ONE //clamp depth between 0.0 to 1.0
#include <glm.hpp>
...
private_mem:
    ...
    VkDescriptorSetLayout descriptorSetLayout; //descriptor layout setting struct
    VkDescriptorPool descriptorPool; //descriptor pool
    VkDescriptorSet descriptorSet; //descriptor pool set to allocate memory
    VkBuffer uniformBuffer; //uniform buffer
    VkDeviceMemory uniformBufferMemory; //uniform buffer memory
    ...
    std::vector<VkFramebuffer> swapChainFramebuffers; //swap chain framebuffer for every image view
    ...
    VkImageView textureImageView; //access to texture image through image view
    VkSampler textureSampler; //texture image sampler
    ...
    VkDeviceMemory vertexBufferMemory; //vertex memory memory
    VkBuffer indexBuffer; //index buffer handle
...
private_fun:
    ...
    bool isDeviceSuitable(VkPhysicalDevice device)
    {
        ...
        return indices.isComplete() && extensionsSupported && swapChainAdequete && supportedFeatures.samplerAnisotropy;
    }
    ...
    void createDescriptorSetLayout()
    {
        ...
        uboLayoutBinding.descriptorCount = 1; //ubo amount
        uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT; //only use in vertex shader
    ...
    void createGraphicsPipeline()
    {
        ...
        rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE; //clockwise or unclockwise
    ...
    void createVertexBuffer()
    {
        ...
        createBuffer(
            bufferSize,
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT, //use staging buffer as a temporary cache in CPU, as a source memory copy to GPU memory
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, //the buffer in CPU host
            stagingBuffer,
            stagingBufferMemory
                    );
        ...
        createBuffer(
            bufferSize,
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, //vertex buffer as a destination memroy accept from another memroy buffer
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, //the buffer in GPU server
            vertexBuffer,
            vertexBufferMemory
                    );
        ...
        //createBuffer(
        //    bufferSize, 
        //    VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, //the buffer as a vertex buffer in CPU
    ...
    uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
    {
        ...
        throw std::runtime_error("Failed to find suitable memroy type.");
    }
    ...
    void createTextureImage()
    {
        ...
        transitionImageLayout(
            textureImage,
            VK_FORMAT_R8G8B8A8_UNORM,
            VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, //format for transfer
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL //format for sampler
                             );
    ...
    void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
    {
        ...
        VkImageMemoryBarrier barrier = {}; //to synchronize the image access
    ...
    void drawFrame()
    {
        ...
        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(device, swapChain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex); //get a image in swap chain with semaphore sync
        ...
        result = vkQueuePresentKHR(presentQueue, &presentInfo); //present opt
    ...
    void updateUniformBuffer()
    {
        ...
        ubo.proj[1][1] *= -1; //invert the Y image bcz GLM is designed for OpenGL
